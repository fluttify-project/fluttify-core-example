...
class MAPolylineRenderer extends MAOverlayPathRenderer {
  ...
  Future<MAPolyline> get_polyline() async {
    final result = await _channel
        .invokeMethod("MAPolylineRenderer::get_polyline", {'refId': refId});
    return result;
  }

  Future<bool> get_is3DArrowLine() async {
    final result = await _channel.invokeMethod(
        "MAPolylineRenderer::get_is3DArrowLine", {'refId': refId});
    return result;
  }

  // 生成setters
  Future<void> set_is3DArrowLine(bool is3DArrowLine) async {
    await _channel.invokeMethod('MAPolylineRenderer::set_is3DArrowLine',
        {'refId': refId, "is3DArrowLine": is3DArrowLine});
  }
  ...
  Future<MAPolylineRenderer> initWithPolyline(MAPolyline polyline) async {
    ...
    // 调用原生方法
    final result = await _channel.invokeMethod(
        'MAPolylineRenderer::initWithPolyline',
        {"polyline": polyline.refId, "refId": refId});
