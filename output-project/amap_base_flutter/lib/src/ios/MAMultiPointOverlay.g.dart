...
class MAMultiPointOverlay extends MAShape with MAAnnotation, MAOverlay {
  ...
  Future<List<MAMultiPointItem>> get_items() async {
    ...
    kNativeObjectPool.addAll((result as List).cast<int>().map((it) => MAMultiPointItem()..refId = it..tag = 'amap_base_flutter').toList());
    return (result as List).cast<int>().map((it) => MAMultiPointItem()..refId = it..tag = 'amap_base_flutter').toList();
  }
